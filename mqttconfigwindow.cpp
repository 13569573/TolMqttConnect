#include "mqttconfigwindow.h"
#include "ui_mqttconfigwindow.h"
#include "mqttmodel.h"
...
MqttConfigWindow::MqttConfigWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MqttConfigWindow)
    , m_model(new MqttModel(this))
{
    ...
    connect(ui->saveButton, &QPushButton::clicked, this, &MqttConfigWindow::onSaveClicked);
    connect(ui->restoreButton, &QPushButton::clicked, this, &MqttConfigWindow::onRestoreClicked);
    connect(ui->importButton, &QPushButton::clicked, this, &MqttConfigWindow::onImportClicked);
...
MqttConfigWindow::~MqttConfigWindow()
{
    delete ui;
}

void MqttConfigWindow::onSaveClicked()
{
...
void MqttConfigWindow::onRestoreClicked()
{
    if (QFile::exists(m_filePath)) {
        m_model->loadFromFile(m_filePath);
...
void MqttConfigWindow::onImportClicked()
{
...
void MqttConfigWindow::onButtonClicked(const QModelIndex &index)
{
    if (!index.isValid())
        return;
